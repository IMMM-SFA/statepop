...
def make_dataframes(file_list):
    dfs = [pd.read_csv(files, skiprows=3) for files in file_list]
...
# Variables (already in dataframe as columns, come back to fix later)
...
scenario_df = scenario_df.loc[scenario_df["year"].isin(list(range((year_start, year_through + timestep, timestep))]

### Continued on debugging.ipynb for now ###