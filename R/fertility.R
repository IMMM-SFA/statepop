...
#' @param useBrassf       (Boolean) Should the Brass Relational Model be used or a simple scaling approach to compute future fertility schedules
...
fertility <- function(inputsPath, regUAll, yearStart=2010, yearEnd=2100,
  ...
  for (regU in 1:length(regUAll)){
    ...
    if (cur.scenario != "Constant_rate"){
      ...
      tfrDf1R <- f.linIntE(tfrER, "tfr", si=T, steps=steps) # rural; si=T first value is starting value; si=F last value is starting value
      tfrDf1U <- f.linIntE(tfrEU, "tfr", si=T, steps=steps) # urban
    }
